...
class Block {
    constructor(blockIndex, data, timestamp, hash, previousHash, difficulty, nonce) {
...
const calculateHash = (blockIndex, data, timestamp, previousHash, difficulty, nonce) => {
    return CryptoJS.SHA256(`${blockIndex + data + timestamp + previousHash + difficulty + nonce}`).toString();
}

const createGenesisBlock = () => {
    const genesisBlock = new Block(0, 'The Times 03/Jan/2009 Chancellor on brink of second bailout for banks', 1652259600, 0, 0, 1, 0);

    genesisBlock.hash = calculateHash(genesisBlock.blockIndex, genesisBlock.data, genesisBlock.timestamp, genesisBlock.previousHash, genesisBlock.difficulty, genesisBlock.nonce);
...
const mineBlock = async (blockData) => {
    ...
    if(addBlock(newBlock, getLatestBlock())) {
        await pool.query(`INSERT INTO getblock(blockIndex, data, timestamp, hash, previousHash, difficulty, nonce) VALUES(${newBlock.blockIndex},'${blockData}', '${newBlock.timestamp}', '${newBlock.hash}','${newBlock.previousHash}',${newBlock.difficulty}, ${newBlock.nonce})`);
    } else {
...
const isValidBlockStructure = (newBlock) => {
    if(typeof(newBlock.blockIndex) === 'number' 
...
const findNonce = (blockIndex, data, timestamp, previousHash, difficulty) => {
    ...
    while(true) {
        let hash = calculateHash(blockIndex, data, timestamp, previousHash, difficulty, nonce);
...
const dbBlocks = async () => {
    let [dbBlocks] = await pool.query(`SELECT * FROM getblock`)
